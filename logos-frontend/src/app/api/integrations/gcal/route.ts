// app/api/integrations/gcal/route.ts
...
async function initiateOAuthFlow(request: NextRequest) {
  ...
  const redirectURI = `${baseUrl}/api/integrations/gcal`;
  ...
  cookiesInstance.set("oauth_state_gcal", state, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    maxAge: 60 * 10, // 10 minutes
    path: "/",
  });
  cookiesInstance.set("oauth_referrer_gcal", referrer, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    maxAge: 60 * 10, // 10 minutes
    path: "/",
...
async function handleOAuthCallback(
  ...
  const cookiesInstance = await cookies();
  const storedState = cookiesInstance.get("oauth_state_gcal")?.value;
  ...
  const storedReferrer =
    cookiesInstance.get("oauth_referrer_gcal")?.value || "/home/integrations";
  cookiesInstance.set("oauth_state_gcal", "", { maxAge: 0, path: "/" });
  cookiesInstance.set("oauth_referrer_gcal", "", { maxAge: 0, path: "/" });
  ...
  try {
    ...
    await TokenService.storeTokens(userId, "gcal", tokens, supabase);
    ...
    redirectUrl.searchParams.set("provider", "gcal");
