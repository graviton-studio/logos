...
async function initiateOAuthFlow(request: NextRequest) {
  ...
  const redirectURI = `${baseUrl}/api/integrations/gsheets`;
  ...
  cookiesInstance.set("oauth_state_gsheets", state, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    maxAge: 60 * 10, // 10 minutes
    path: "/",
  });
  cookiesInstance.set("oauth_referrer_gsheets", referrer, {
    httpOnly: true,
    secure: process.env.NODE_ENV === "production",
    maxAge: 60 * 10, // 10 minutes
    path: "/",
...
async function handleOAuthCallback(
  ...
  const cookiesInstance = await cookies();
  const storedState = cookiesInstance.get("oauth_state_gsheets")?.value;
  ...
  const storedReferrer =
    cookiesInstance.get("oauth_referrer_gsheets")?.value ||
    "/home/integrations";
  cookiesInstance.set("oauth_state_gsheets", "", { maxAge: 0, path: "/" });
  cookiesInstance.set("oauth_referrer_gsheets", "", { maxAge: 0, path: "/" });
  ...
  try {
    ...
    await TokenService.storeTokens(userId, "gsheets", tokens, supabase);
    ...
    redirectUrl.searchParams.set("provider", "gsheets");
